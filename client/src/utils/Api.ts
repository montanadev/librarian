...
export class Api {
    ...
    uploadDocuments(file: any) {
        return axios.post(`/api/documents/${file.name}`, file).then(d => d.data).catch((error: any) => {
    ...
    renameFolder(folderId: number, newFolderName: string) {
        return axios.put(`/api/folders/${folderId}`, {id: folderId, name: newFolderName, documents: []});
    }

    renameDocument(documentId: number, newDocumentName: string) {
        return axios.put(`/api/documents/${documentId}/details`, {id: documentId, filename: newDocumentName});
    }
}
