...
class TestFolderViews(TestCase):
    ...
    def test_delete_folder(self):
        url = reverse('folder-create')
        folder = self.client.post(url, {'name': 'new-folder', 'documents': []}, format="json")
        self.assertEqual(folder.status_code, status.HTTP_201_CREATED)

        response = self.client.delete(reverse('folder-detail', args=(folder.json()['id'],)))
    ...
    def test_add_document_to_folder(self):
        ...
        response = self.client.put(url, {'id': self.test_doc.json()['id']}, format="json")
